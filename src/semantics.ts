...
export class ArucasSemanticTokenProvider implements vscode.DocumentSemanticTokensProvider {
    ...
    private visitChildren(node: Node, builder: vscode.SemanticTokensBuilder) {
        node.children().forEach((child) => {
            ...
            const type = tk.type;
            if (type) {
                builder.push(tk.token.trace.lineStart, tk.token.trace.columnStart, tk.token.trace.length, this.encodeTokenType(type), this.encodeTokenModifiers(tk.modifiers));
            }
